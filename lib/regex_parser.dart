...
class Tuple2<T1, T2> {
  ...
  bool operator ==(Object other) =>
      other is Tuple2<T1,T2> && other.item1 == this.item1 && other.item2 == this.item2;
...
static Map<String, String> getClassesText(String text) {
  ...
  for (Match match in classNameExp.allMatches(text)) {
    classNames.add(Tuple2(text.substring(match.start + 6).split(' ')[0], text.substring(match.start).indexOf("{") + match.start - 1));
  }
  ...
  var seen = <int>{};
  classNames = classNames.where((element) => seen.add(element.item2)).toList();
  ...
  classes.forEach((className, classText) {
    ...
    List<String> extractVariableNames(String code, String nameOfClass) {
      ...
      for (String statement in statements) {
        ...
        if (statement.substring(statement.indexOf(name)).contains('(') && !statement.contains('=')) {
          continue;
        }
