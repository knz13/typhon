...
import '../../presentation/engine_frontend.dart';
...
class _FileViewerPanelState extends State<FileViewerPanel> {
  ...
  Future<void> _refreshWatchers(Directory dir) async {
    ...
    for (var file in dirs) {
      if (file is File &&
        ...
        var fileWatcher = FileWatcher(file.absolute.path,
  ...
  Widget _buildBreadcrumbTrail() {
    ...
    for (final index in List.generate(breadCrumbs.length, (index) => index)) {
      breadcrumbs.add(const HorizontalSpacing(10));
    ...
    breadcrumbs.add(const SizedBox(
      width: 10,
    ));
                  ...
                  child: GestureDetector(
                    onSecondaryTap: () {
                      ...
                      showNativeContextMenu(
                          context,
                          EngineFrontend.mousePosition.dx,
                          EngineFrontend.mousePosition.dy, [
                        if (hoveringPath != "")
                          ContextMenuOption(
                              title: "Open",
                              callback: () {
                                launchUrl(Uri.parse(
                                    "file:${path.join(FileViewerPanel.currentDirectory.value.absolute.path, hoveringPath)}"));
                              }),
                        ContextMenuOption(
                            title: "Open Folder In Explorer",
                            callback: () {
                              launchUrl(Uri.parse(
                                  "file:${FileViewerPanel.currentDirectory.value.absolute.path}"));
                            }),
